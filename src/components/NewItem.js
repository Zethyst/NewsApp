...
export class NewItem extends Component {
  render() {
    ...
    let d=new Date(date);
    return (
        <div className="relative mx-8 my-6 rounded-2xl flex flex-col justify-center items-center md:w-64 transition-all duration-500 ease-out hover:scale-105" style={{backgroundColor:"rgb(255 255 255)",height:"340px"}}>
          <span className="absolute -top-3 left-1/2 transform -translate-x-1/2  badge rounded-pill bg-danger">{source}</span>
        <img src={imageURL?imageURL:"https://img.freepik.com/premium-vector/default-image-icon-vector-missing-picture-page-website-design-mobile-app-no-photo-available_87543-11093.jpg?w=740"} className="h-32 md:w-52 w-56 rounded-lg" alt="Image"/>
        <div className="flex flex-col my-2 md:w-48 w-52">
